function [time_full,conc_full,time_norm,conc_norm,time_end,conc_end,avg_ep,eplen,epochstep,epochmax,philist,taulist,final_epoch,br,dr,alp,bet,gam,del,fomut,bamut] = glenn_epoch_runner_transfer(eplen,epochstep,epochmax,philist,taulist,br,dr,alp,bet,gam,del,fmut,bmut,inits,tolrnc)
% Solves simple serial transfer experiment competition equation (with transfer sizes drawn from a lognormal distribution) of phage strains differing in lysogeny rates and decision mechanisms of a set of epochs, using glenn_solver().
    ...
    in=inits(2+num:end);
    mu = log(((sum(in))^2)/sqrt((sum(in)/10)+(sum(in))^2));
    sigma = sqrt(log((sum(in)/10)/((sum(in))^2)+1));
    ...
    while epoch<epochmax && close_nuff < 100*(eplen/dt)*(1+2*num)
        ...
        y = glenn_solver(b,d,alpha,beta,gamma,delta,phis,taus,fomut,bamut,t,init_x');      
        y(y<0)=0;
        ...
        % Determine the size of the transfer (trsz)
        trsz = lognrnd(mu,sigma);
        
        % Create a new starting vector
        init_x(1) = .7;
        init_x(2:(1+num)) = 0;
        init_x((2+num):end) = z(end,(2+num):end) .* trsz;
